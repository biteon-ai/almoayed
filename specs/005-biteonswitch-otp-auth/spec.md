...
- Q: When registration creates the student profile → A: Create + link profile on successful registration submit; OTP only mints the session
- Q: Public registration URL strategy → A: `/login` is registration primary; existing-user OTP CTA on same page; `/register` optional alias to same experience
- Q: Device session lock for admin fallback → A: AUTH-003 applies; new fallback sign-in invalidates the prior session
- Q: Students/teachers when BiteonSwitch is down → A: Arabic outage + retry only; no emergency bypass for ordinary users; admin fallback remains privileged-only
...
5. **Given** BiteonSwitch is down or unreachable, **When** a student or ordinary teacher tries to start or complete OTP sign-in, **Then** they see a clear Arabic outage/retry message and are not offered any emergency secret or alternate password login.

---

### User Story 2 - New Student Registers with Teacher Code (Priority: P1)

A new student uses the public page as a **registration** form: full name, WhatsApp number, and teacher code. On success the student profile is created and linked to that teacher immediately; they are then guided to complete BiteonSwitch OTP so a session can be minted (registration alone does not sign them in). Existing users see clear Arabic guidance that registration is not for them and how to sign in via OTP instead.
...
**Independent Test**: Submit valid name + WhatsApp + known teacher code for a new student; profile and teacher link exist before OTP; user is directed to OTP sign-in and is not treated as signed in until OTP succeeds. Invalid teacher code shows a clear Arabic error and does not create a misleading “signed in” state.

**Acceptance Scenarios**:

1. **Given** a valid teacher code and a WhatsApp number not yet registered, **When** the student submits registration, **Then** a student profile is created and linked to that teacher immediately, and the student is guided to BiteonSwitch OTP to mint a session (they are not signed in yet).
...
4. **Given** a visitor on `/login` (or optional `/register` alias), **When** they are an existing user, **Then** they can find a prominent, RTL-friendly path to “sign in with WhatsApp OTP” without filling the registration form.
...
3. **Given** an admin already signed in on device A, **When** the same privileged identity completes admin fallback on device B, **Then** device A’s session is no longer valid and device B becomes the active session (AUTH-003).
4. **Given** a student on the public registration/sign-in page, **When** they view the page, **Then** admin fallback fields and the admin route are not advertised or embedded in the student UI.
5. **Given** BiteonSwitch is unreachable, **When** an admin uses fallback successfully, **Then** they can still perform essential teacher/admin work without OTP.
...
- Verification succeeds for a WhatsApp number with no profile and no prior registration: user is told to register with a teacher code first (or equivalent safe guidance), and no privileged session is minted.
- Student completes registration but abandons OTP: profile and teacher link remain; they can later sign in via OTP without re-registering.
...
- Network timeout or provider outage while contacting BiteonSwitch: students and teachers see a recoverable Arabic outage/retry message and have no emergency bypass; admin fallback remains available only on its dedicated privileged entry.
...
- **FR-004**: After BiteonSwitch reports successful verification, the system MUST resolve the matching existing profile, enforce single-device session lock (AUTH-003), establish a server-side session with role and active teacher context when applicable, and redirect by role to student home or teacher home. OTP verification MUST NOT be the step that creates the student–teacher registration link.
- **FR-005**: New student registration MUST validate the teacher code and, on success, immediately create the student profile and link them to that teacher; registration alone MUST NOT mint a signed-in session (AUTH-002 preserved). OTP is required afterward to sign in.
...
- **FR-009**: Admin fallback success MUST grant access to teacher/admin controls even when BiteonSwitch is down or unreachable, and MUST enforce the same single-device session lock (AUTH-003) as OTP-minted sessions.
...
- **FR-014**: The public registration entry MUST remain at `/login` (primary, bookmark-stable). The same page MUST include a distinct existing-user action to start BiteonSwitch WhatsApp OTP. An optional `/register` path MAY alias to the same registration experience; it MUST NOT become a second divergent registration flow.
- **FR-015**: When BiteonSwitch is unavailable, the system MUST show students and teachers a clear Arabic outage message with a retry path and MUST NOT offer them any emergency secret, shared code, or alternate password login. Only the dedicated admin fallback (AUTH-005) may bypass OTP.
...
- **SC-002**: 100% of new-student registrations with a valid teacher code result in an immediate teacher link and a clear next step to complete OTP for sign-in (no silent dead-end; registration alone does not leave the user signed in).
...
- **SC-006**: Device session lock continues to hold for both OTP and admin fallback: after a second successful sign-in on either path, the previous session is rejected on next protected action in 100% of test runs.
...
- `/login` is the primary public registration URL (backward-compatible). `/register` may alias to the same experience. Existing users start OTP from a CTA on that page rather than a separate mandatory login page.
- Successful student registration creates and links the profile immediately; BiteonSwitch OTP is required afterward only to mint the session. Abandoned OTP does not undo the registration link.
...
- Admin fallback is for operational continuity, not for student or ordinary teacher daily login; during BiteonSwitch outages, ordinary users get outage/retry messaging only (no general emergency bypass).
- Local/demo identities remain a controlled exception for development and demos and are not a production outage bypass for real students/teachers.
- AUTH-003 single-device lock applies equally to OTP-minted sessions and admin fallback sessions; a new successful sign-in (either path) invalidates the previous device session.
